...
import (
	...
	"github.com/liquid-dev/text/collatee
...
`
	...
	copyExported("github.com/liquid-dev/net/idna")
...
func copyExported(p string) {
	copyPackage(
		...
		filepath.Join("..", filepath.FromSlash(p[len("github.com/liquid-dev"):])),
		"github.com/liquid-dev/text/internal/export/"+path.Base(p),
		p)
}
